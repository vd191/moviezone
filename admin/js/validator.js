...
function validateSignUpForm(checkEditMode,memberid){
    var editing_mode = checkEditMode;
    ...
    if (surname.match(charRegex) ){
        ...
        err = err + "Surname should contain characters only ";
    }
    ...
    if (othername.match(charRegex) ){
        ...
        err = err + "Othername should contain characters only";
    }
    ...
    if (occupation.match(charRegex) ){
        ...
        err = err + "Occupation should contain characters only ";
    }
    ...
    if(contactMethod == "phone"){
        if (phone.match(phoneRegex) ){
            ...
            err= err + " Phone format incorrect";
        }    
    }
    ...
    if(contactMethod == "landline"){
        if (landline.match(landlineRegex) ){
            ...
            err = err + "Landline Format incorrect ";
        }
    }
    ...
    if(contactMethod == "email"){
        if (email.match(emailRegex) ){
            ...
            err = err + "Email Format incorrect ";
        }
    }
    ...
    if(magazine == 1){
        ...
        if (street.match(streetRegex)) {
            ...
            err = err + "Street Format incorrect ";
        }
        ...
        if (suburb.match(charRegex) ){
            ...
            err = err + "suburb Format Correct: Suburb, State ";
        }
        ...
        if (postcode.match(postcodeRegex)){
            ...
            err = err + "postcode should contain 4 number ";
        }
    }
    ...
    if (username.match(userNameRegex) ){
        ...
        err = err + "username Format incorrect ";
    }
    ...
    if(err == ""){
        if(editing_mode){
            alert('editing mode');
            params +='&edituserid='+memberid;
            alert(params);
            makeAjaxPostRequest('admin_main.php', 'cmd_member_edit', params, updateContent); 
        }else{
            alert(params);
            makeAjaxPostRequest('admin_main.php', 'cmd_member_register', params, function(){
